(() => {
  function Datera(
    ...
    options.value && (this.value = options.value.map((val) => new Date(val))),
      options.minYear && (this.minYear = options.minYear),
      options.maxYear && (this.maxYear = options.maxYear),
      ...
      options.startWeek && (this.startWeek = options.startWeek),
      options.daysOfWeek && (this.daysOfWeek = options.daysOfWeek),
      options.selectionType && (this.selectionType = options.selectionType),
      options.theme && (this.theme = options.theme),
      options.divider && (this.divider = options.divider),
      (this.input =
        "string" == typeof input
          ? document.querySelector(input) ||
            ...
            document.querySelector("#" + input)
          : input),
      this.input && (this.input.readOnly = !0);
  }
  (Datera.prototype = {
    input: null,
    value: [],
    ...
    position: [],
    mount() {
      this.bindEvents(), this.getCalendarDays();
    },
    getGridDays() {
      var startCurrentDay = new Date(
        ...
        startCurrentDay = new Date(startCurrentDay);
      startCurrentDay.setDate(
        startCurrentDay - getDate() - startCurrentDay.getDay()
      );
    },
    placeCalendar() {
    ...
    bindEvents() {
      this.input.addEventListener("click", () => this.showCalendar()),
        ...
        window.addEventListener("scroll", () => {
          this.calculatePosition(), this.placeCalendar();
        }),
    ...
    hideCalendar(event) {
      this.calendarDOM.contains(event.target) ||
        ...
        this.calendarDOM.classList.contains("hidden") ||
    ...
    displayValue() {
      "single" == this.selectionType && this.value[0]
        ? (this.input.value = this.formatDisplayDate(this.value[0]))
        : (("range" == this.selectionType && this.value[0] && this.value[1]) ||
            ("list" == this.selectionType && this.value.length)) &&
    ...
    getAvailableYears() {
      return (
        this.minYear || (this.minYear = new Date().getFullYear() - 100),
        this.maxYear || (this.maxYear = new Date().getFullYear() + 101),
        new Array(this.maxYear - this.minYear + 1)
          .fill(null)
    ...
    generateCalendarWeeks(from, to) {
      var days = this.generateCalendarDays(from, to),
        groupedWeeks = [];
      for (let i = 0; i < days.length / 7; i++)
        for (let j = 0; j < 7; j++)
          groupedWeeks[i] || (groupedWeeks[i] = []),
            (groupedWeeks[i][j] = days[7 * i + j]);
      return groupedWeeks;
    },
    changeMonth(step) {
      1 == step
    ...
    showCalendar() {
      this.calculatePosition(),
        this.calendarDOM || (this.calendarDOM = document.createElement("div")),
        // console.log(this.canNavigateBack(), this.canNavigateNext()),
        (this.calendarDOM.classList = "datera--calendar " + this.theme),
        (this.calendarDOM.innerHTML = `<div class="datera--controls"><button class="datera--month-decrement" ${
          this.canNavigateBack() ? "" : "disabled"
        }><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-chevron-left"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M15 6l-6 6l6 6" /></svg></button><div class="datera--control-selects"><select class="datera--month-select">${this.months.map(
          (month, i) =>
            `<option ${
              this.currentDate.getMonth() == i ? "selected" : ""
            } value="${i}">${month}</option>`
        )}</select><select class="datera--year-select">${this.getAvailableYears().map(
          (year, i) =>
            `<option ${
              this.currentDate.getFullYear() == year ? "selected" : ""
            } value="${year}">${year}</option>`
        )}</select></div><button class="datera--month-increment" ${
          this.canNavigateNext() ? "" : "disabled"
        }><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-chevron-right"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 6l6 6l-6 6" /></svg></button></div><div class="datera--grid">${new Array(
          7
        )
          .fill("")
          .map(
            (_, i) => `<p class="datera--day-of-week">${this.daysOfWeek[i]}</p>`
          )
          .join("")}</div><div class="datera--grid days"></div>`),
        this.calendarDOM
        ...
        this.calendarDOM
          ...
          .addEventListener("click", (e) => {
            e.stopPropagation(), this.changeMonth(-1), this.showCalendar();
          }),
        this.calendarDOM
        ...
        this.renderCalendar(),
        this.placeCalendar(),
        this.calendarDOM.parentNode || document.body.append(this.calendarDOM);
    },
    ...
    renderCalendar() {
      (this.calendarDOM.querySelector(
        ".datera--grid.days"
      ).innerHTML = `${this.getCalendarDays()
        .map(
          (d) =>
            `<button ${
              ...
              "single" == this.selectionType &&
              this.value[0]?.toDateString() == d.toDateString()
                ? "selected"
                : ""
            } ${
              "list" == this.selectionType &&
              this.value.find((val) => val.toDateString() == d.toDateString())
                ? "selected"
                : ""
            }  ${
              "range" == this.selectionType
                ? `${
                    this.value[1] &&
                    this.value[0]?.toDateString() == d.toDateString()
                      ? "start-range"
    ...
    onDatePicked(selectionDate) {
      ...
      if ("range" == this.selectionType)
        if (this.value[0]) {
          if (this.value[1])
            (this.value[0] = decodedSelectionDate), (this.value[1] = void 0);
          else if (
            !this.value[1] &&
            ...
            betweenDates
              ...
              : ((this.value[1] = decodedSelectionDate),
                this.value[0] > this.value[1] && this.value.reverse());
          }
        } else this.value[0] = decodedSelectionDate;
      else
        "single" == this.selectionType
          ? (this.value = [decodedSelectionDate])
          : "list" == this.selectionType &&
            (-1 <
            (selectionDate = this.value.findIndex(
              (val) => val.toDateString() == decodedSelectionDate.toDateString()
            ))
              ? this.value.splice(selectionDate, 1)
